...
public class RecondClass extends JFrame implements MouseListener{
    ...
    private static volatile RecondClass instance;
    //当使用时才创建
    public static RecondClass  getInstance(){
        if (instance == null){
            synchronized(RecondClass.class){
                if (instance == null){
                    instance = new RecondClass();
                }
            }
        }
        return instance;
    ...
    public void mouseReleased(MouseEvent e) {
        ...
        if(e.getSource().equals(captureBtn))
        {
            ...
            save();
            //将其放到指定的路径下
            //定义最终要存放的文件路径Program Files/apache-tomcat-6.0.35/webapps/XWZ/tempFile/
    ...
    public void save()
    {
        ...
        try {
            //以当前的时间命名录音的名字
            //将录音的文件存放到F盘下语音文件夹下
            File filePath = new File("D:/test/AudioFile");
            String tarPath = "D:/";
            if(!filePath.exists())
            {//如果文件不存在，则创建该目录
                filePath.mkdirs();
            ...
            String name = "recond";
            file = new File(filePath+"/"+name+".wav");
            AudioSystem.write(ais, AudioFileFormat.Type.WAVE, file);
            //将录音产生的wav文件转换为容量较小的mp3格式
            //定义产生后文件名
            tarFile = new File(tarPath+name+".mp3");
            Runtime run = null;
            //测试当前的路径
